    ...
    if infra:
        ...
        for ep in list_network_connector_endpoints(k5token, region).json()['network_connector_endpoints']:
            if len(show_network_connector_ep_interfaces(k5token, ep.get('id'), region).json()['network_connector_endpoint']['interfaces']) > 0:
                portid = show_network_connector_ep_interfaces(k5token, ep.get('id'), region).json()['network_connector_endpoint']['interfaces'][0].get('port_id')
                print disconnect_network_connector_endpoint(k5token, ep.get('id'), portid, region)
                print delete_port(k5token, portid, region)
...
def main():
    ...
    purge_project(demoProjectAid, demoProjectBid, deleteinfrastructure)
